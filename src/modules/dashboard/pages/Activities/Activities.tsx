...
export default function Activities() {
  ...
  const {
    Activitieslist,
    isSuccess,
    isInsertActivitiesSuccess,
    isUpdateActivitiesSuccess,
    isDeleteActivitiesSuccess,
    isError,
    isInsertActivitiesError,
    insertActivitiesErrorMessage,
    isUpdateActivitiesError,
    isDeleteActivitiesError,
    updateActivitiesErrorMessage,
    deleteActivitiesErrorMessage,
    isDataSuccess,
    isDataError,
    ActivitiesData,
  } = useSelector(ActivitieslistSelector);
  ...
  useEffect(() => {
    dispatch(getActivitieslist({ token }));
  ...
  useEffect(() => {
    ...
    if (isInsertActivitiesSuccess) {
      toast.success("Successfully saved.");
    }

    if (isUpdateActivitiesSuccess) {
      toast.success("Successfully updated.");
    }

    if (isDeleteActivitiesSuccess) {
      toast.success("Successfully deleted.");
    }

    dispatch(clearState());
  }, [isInsertActivitiesSuccess, isUpdateActivitiesSuccess, isDeleteActivitiesSuccess]);

  useEffect(() => {
    if (isSuccess) {
      setSearchdata(Activitieslist);
  ...
  }, [isSuccess, isError, Activitieslist]);

  useEffect(() => {
    if (isDataSuccess) {
      setValue("date", dayjs(ActivitiesData[0].publish_date).format("YYYY-MM-DD"));
      setValue("title", ActivitiesData[0].en_title);
      setValue("thai_title", ActivitiesData[0].thai_title);
      setValue("sContent", ActivitiesData[0].en_full_content);
      sEditor?.setData(ActivitiesData[0].en_full_content);
      setValue("fContent", ActivitiesData[0].thai_full_content);
      fEditor?.setData(ActivitiesData[0].thai_full_content);
      dispatch(clearState());
      const arraybuffer = (ActivitiesData[0].image) ? ActivitiesData[0].image : "";
  ...
  }, [isDataSuccess, isDataError, ActivitiesData]);

  useEffect(() => {
    if (isInsertActivitiesError) {
      toast.error(`Save failed`);
      dispatch(clearState());
    }
    if (isUpdateActivitiesError) {
      toast.error(`Update failed`);
      dispatch(clearState());
    }
    if (isDeleteActivitiesError) {
      toast.error(`Delete failed`);
      dispatch(clearState());
    }
  }, [isInsertActivitiesError, isUpdateActivitiesError, isDeleteActivitiesError]);

  const handleEdit = (id: number) => {
    dispatch(getActivitiesById({ token, id }));
    setButton(false);
    setEditId(id);
  };

  const handleDelete = (id: number) => {
    dispatch(deleteActivities({ token, id }));
    reset();
    sEditor.setData();
    fEditor.setData();
    setButton(true);
          ...
          <Grid container item xs={12} spacing={5}>
            <Grid item container xs={12} spacing={0}>
              <form
                ...
                noValidate
                onSubmit={handleSubmit(submitActivities)}
              >
